...
const PREVIEW_INSIGHTS = [
	{
		...
		description:
			"75% of your **Sugar** cravings happen when you're **Stressed**.",
		actionable:
			'Deep breathing for 2 minutes, take a walk, or text a friend.',
		type: 'Emotional Pattern',
	},
	{
		title: 'Time Pattern • Willpower',
		description:
			'You resist **80%** in **morning** but only **20%** at **night**.',
		actionable:
			'Schedule important tasks in the morning when willpower is strongest.',
		type: 'Time-Based',
	},
	{
		title: 'Substitution • Instagram → TikTok',
		description:
			'60% of the time you resist **Instagram**, you reach for **TikTok** within 2 hours.',
	...
	},
	{
		...
		description:
			'You wrote **"tired"** in 40% of reflections, always followed by **YouTube** cravings.',
		actionable:
			'Sleep more or take a real nap instead of reaching for quick fixes.',
		type: 'Root Cause',
	},
]
...
export default function Onboarding() {
	...
	const handleSuccessContinue = () => {
		setCurrentScreen(4) // Go to emotions selection screen
	}

	const handleEmotionsContinue = async () => {
		// Save custom emotions and go to stats preview
		await setCustomEmotions(selectedEmotions)
		setCurrentScreen(5) // Go to stats preview screen
	}

	const isLastScreen = currentScreen === 5
			...
			return (
				<View className='flex-1 justify-center px-6'>
					...
					<Text className='text-base text-slate-600 text-center leading-relaxed'>
						A gentle approach to building resistance against digital
						distractions and unhealthy habits.
					</Text>
				</View>
			)
		}
			...
			return (
				...
				>
					...
					<Text className='text-sm text-slate-600 text-center mb-8'>
						Select at least One Craving to get started!
					</Text>
				...
				>
					...
					<View className='flex-row gap-3 mb-6'>
						{selectedCravings.slice(0, 3).map((craving) => {
							const cat = QUICK_CATEGORIES.find(
								(c) => c.name === craving,
							) || {
								name: craving,
								icon: Cookie,
							...
							return (
								...
								>
									...
									<Text
										className={`text-xs font-medium mt-2 ${isSelected ? 'text-indigo-700' : 'text-slate-700'}`}
									>
										{craving}
									</Text>
								</TouchableOpacity>
					...
					>
						<View className='flex-row items-center justify-between'>
							<View>
								<Text
									className={`font-bold text-base ${selectedType === 'observed' ? 'text-amber-700' : 'text-slate-900'}`}
								>
									I just noticed it
								</Text>
					...
					>
						<View className='flex-row items-center justify-between'>
							<View>
								<Text
									className={`font-bold text-base ${selectedType === 'resisted' ? 'text-emerald-700' : 'text-slate-900'}`}
								>
									I resisted it!
								</Text>
								<Text className='text-xs text-slate-600 mt-1'>
			...
			return (
				<View className='flex-1 justify-center px-6'>
					...
					<Text className='text-base text-slate-600 text-center leading-relaxed mb-4'>
						Keep tracking to unlock badges, discover patterns, and
						build lasting habits.
					</Text>
					<Text className='text-sm text-slate-500 text-center'>
						Both noticing AND resisting build awareness.{'\n'}You're
						growing stronger either way.
					</Text>
				</View>
			)
		}
				...
				<View className='gap-4 mb-6'>
					...
					>
						<View className='items-center mb-4'>
							...
							<Text className='text-slate-600 text-sm'>
								Progress through 5 levels as you grow
							</Text>
						</View>
						<View className='bg-slate-100 rounded-full h-3 overflow-hidden mb-2'>
							<View
						...
						<Text className='text-slate-500 text-xs text-center'>
							Build awareness and resistance
						</Text>
					</View>
					...
					>
						...
						<Text className='text-slate-600 text-sm leading-5'>
							See detailed breakdowns of observed vs resisted
							moments for each craving category
						</Text>
					</View>
				</View>
			</ScrollView>
			...
			>
				...
				{currentScreen === 0 && (
					<TouchableOpacity
						...
						className='bg-indigo-600 rounded-xl py-3 mb-8 items-center'
						activeOpacity={0.7}
					>
						<Text className='text-white font-bold text-sm'>
							Next
				...
				{currentScreen === 1 && (
					<TouchableOpacity
						...
						disabled={
							selectedCravings.length < 1 ||
							selectedCravings.length > 10
						}
						className={`rounded-xl py-3 mb-8 items-center ${
							selectedCravings.length >= 1 &&
							selectedCravings.length <= 10
								? 'bg-indigo-600'
								: 'bg-slate-300'
				...
				{currentScreen === 4 && (
					<TouchableOpacity
						onPress={handleEmotionsContinue}
						disabled={
							selectedEmotions.length < 1 ||
							selectedEmotions.length > 10
						}
						className={`rounded-xl py-3 mb-8 items-center ${
							selectedEmotions.length >= 1 &&
							selectedEmotions.length <= 10
